...
2. Next, let's set up the first route. Open *index.js* from the "routes" folder. Update the code as follows:
  ...
  *Remember:* Routes have a path (string and/or regex), callback function, and a HTTP method. In the above code, we are simply adding the callback, which render the `index` view and sets a title. 
